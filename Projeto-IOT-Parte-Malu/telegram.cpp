...
void reconectarMQTT()
{
    if (!mqtt.connected())
    {
        ...
        mqtt.subscribe("telegram", 1); // verificar!!
    }
}
...
void recebeuMensagem(String topico, String conteudo)
{
    ...
    if (topico.startsWith("telegram")) // verificar!!
    {
        JsonDocument mensagem;
        deserializeJson(mensagem, conteudo);

        String chatId = mensagem[0];
        String conteudo_mensagem = mensagem[1];
